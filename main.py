...
# 国家代码 -> 中文名称（取映射表中第一个出现的中文名，作为“优选标签”展示用）
CODE_TO_CN = {}
for _cn_name, _code in CN_TO_CODE.items():
    if _code not in CODE_TO_CN:
        CODE_TO_CN[_code] = _cn_name


def country_code_to_flag(code):
    """根据两位国家代码生成 emoji 国旗（如 JP -> 🇯🇵），非法输入返回空字符串"""
    if not code or not re.match(r'^[A-Za-z]{2}$', code):
        return ""
    upper = code.upper()
    return chr(ord(upper[0]) - ord('A') + 0x1F1E6) + chr(ord(upper[1]) - ord('A') + 0x1F1E6)


def extract_node_country_code(node):
    """从节点字符串（如 1.2.3.4:443#JP）中提取两位国家代码；无法识别时返回 None"""
    if '#' not in node:
        return None
    ...
    if not label:
        return None
    label = label.split()[0]
    return extract_country_code(label) or (label.upper() if re.match(r'^[A-Za-z]{2}$', label) else None)
...
IP_TXT_USE_NEW_FORMAT = cfg["IP_TXT_USE_NEW_FORMAT"]
...
def _build_new_format_lines(final_nodes, speed_map, latency_map,
                            http_latency_map=None, http_jitter_map=None):
    """按国家分组生成新格式行：IP:端口#国旗|国家优选序号|[带宽M]|[HTTP延迟ms]|[HTTP抖动ms]|[TCP延迟ms]
    末尾各字段是否打印由 IP_TXT_SHOW_BANDWIDTH / IP_TXT_SHOW_HTTP_LATENCY /
    IP_TXT_SHOW_HTTP_JITTER / IP_TXT_SHOW_LATENCY 四个开关控制。"""
    groups = []
    group_index = {}
    for node in final_nodes:
        ipport = node.split('#')[0]
        code = extract_node_country_code(node)
        key = code if code else (node.split('#', 1)[1] if '#' in node else '')
        if key not in group_index:
            group_index[key] = len(groups)
            groups.append([key, []])
        groups[group_index[key]][1].append((node, ipport))

    lines = []
    for key, nodes in groups:
        code = key if (key and len(key) == 2) else None
        flag = country_code_to_flag(code) if code else ""
        cn = CODE_TO_CN.get(code, key) if code else key
        for i, (node, ipport) in enumerate(nodes, 1):
            line = f"{ipport}#{flag}|{cn}优选{i}"
            if IP_TXT_SHOW_BANDWIDTH:
            ...
            if IP_TXT_SHOW_HTTP_LATENCY and http_latency_map and node in http_latency_map:
                line += f"|{http_latency_map[node]:.0f}ms"
            if IP_TXT_SHOW_HTTP_JITTER and http_jitter_map and node in http_jitter_map:
                line += f"|{http_jitter_map[node]:.0f}ms"
            if IP_TXT_SHOW_LATENCY:
                lat_ms = latency_map.get(node, 0.0) if latency_map else 0.0
                line += f"|{lat_ms*1000:.0f}ms"
            lines.append(line)
    return lines
...
def write_ip_txt(final_nodes, output_file,
                 ...
                 http_latency_map=None, http_jitter_map=None):
    if IP_TXT_USE_NEW_FORMAT:
        body_lines = _build_new_format_lines(final_nodes, speed_map, latency_map,
                                             http_latency_map, http_jitter_map)
    else:
        body_lines = []
        for node in final_nodes:
            line = node
...
def main():
    ...
    if GITHUB_SYNC_ENABLED:
        ...
        print("GitHub 同步已禁用（GITHUB_SYNC_ENABLED=false），跳过推送。")
